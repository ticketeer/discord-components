...
export default function DiscordInteraction({
  ...
  return (
    <div className="discord-interaction">
      ...
      <AuthorInfo className="discord-interaction-author-info" author={highlight ? `@${user.author}` : user.author} bot={user.bot} roleColor={user.roleColor} />
      {command ? (
        <span className="discord-interaction-command">
          used <span className="discord-interaction-command-name">/{children}</span>
        </span>
      ) : (
